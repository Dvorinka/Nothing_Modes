...
class PrioritizedPrivilegedShellTest {
    @Test
    fun `queued commands are single writer and highest priority goes first`() =
        runTest {
    ...
    @Test
    fun `invalid requests are rejected before reaching privileged transport`() =
        runTest {
            var calls = 0
            val shell =
                PrioritizedPrivilegedShell(
    ...
    @Test
    fun `execution id reaches privileged transport and correlated result`() =
        runTest {
    ...
    @Test
    fun `close cancels running and queued commands and rejects new work`() =
        runTest {
            val dispatcher = StandardTestDispatcher(testScheduler)
            val gate = CompletableDeferred<Unit>()
            val shell =
