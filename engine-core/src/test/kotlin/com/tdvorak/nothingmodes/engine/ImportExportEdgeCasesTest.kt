...
class ImportExportEdgeCasesTest {
    ...
    @Test
    fun `import empty JSON string returns parse error`() =
        runTest {
    ...
    @Test
    fun `import empty JSON object returns zero automations`() =
        runTest {
            val store = InMemoryAutomationStore()
            val service = ImportExportService(store) { 1000L }
    ...
    @Test
    fun `import corrupted JSON with truncated structure returns parse error`() =
        runTest {
            val store = InMemoryAutomationStore()
            val service = ImportExportService(store) { 1000L }
    ...
    @Test
    fun `import JSON with garbage bytes returns parse error`() =
        runTest {
            val store = InMemoryAutomationStore()
            val service = ImportExportService(store) { 1000L }
    ...
    @Test
    fun `import wrong schema version v0 returns unsupported error`() =
        runTest {
            val store = InMemoryAutomationStore()
            val service = ImportExportService(store) { 1000L }
    ...
    @Test
    fun `import wrong schema version v999 returns unsupported error`() =
        runTest {
            val store = InMemoryAutomationStore()
            val service = ImportExportService(store) { 1000L }
    ...
    @Test
    fun `import with missing automations field returns parse error`() =
        runTest {
            val store = InMemoryAutomationStore()
            val service = ImportExportService(store) { 1000L }
    ...
    @Test
    fun `import with missing schemaVersion field returns parse error`() =
        runTest {
            val store = InMemoryAutomationStore()
            val service = ImportExportService(store) { 1000L }
    ...
    @Test
    fun `import automation with unknown action type returns parse error`() =
        runTest {
            val store = InMemoryAutomationStore()
            val service = ImportExportService(store) { 1000L }
    ...
    @Test
    fun `import automation with unknown trigger type returns parse error`() =
        runTest {
            val store = InMemoryAutomationStore()
            val service = ImportExportService(store) { 1000L }
    ...
    @Test
    fun `import with duplicate IDs skips second when overwrite is false`() =
        runTest {
            val store = InMemoryAutomationStore()
            val service = ImportExportService(store) { 1000L }
    ...
    @Test
    fun `import overwrites duplicate IDs when overwrite is true`() =
        runTest {
            val store = InMemoryAutomationStore()
            val service = ImportExportService(store) { 1000L }
    ...
    @Test
    fun `import sets enabled to false and createdBy to IMPORT`() =
        runTest {
            val store = InMemoryAutomationStore()
            val service = ImportExportService(store) { 1000L }
    ...
    @Test
    fun `export empty store produces valid JSON with zero count`() =
        runTest {
            val store = InMemoryAutomationStore()
            val service = ImportExportService(store) { 1000L }
    ...
    @Test
    fun `export by non-existent IDs produces zero count`() =
        runTest {
            val store = InMemoryAutomationStore()
    ...
    @Test
    fun `import-export round trip with complex nested conditions`() =
        runTest {
            val store = InMemoryAutomationStore()
            val original =
                Automation(
                    ...
                    conditions =
                        com.tdvorak.nothingmodes.engine.model.Condition.And(
                            listOf(
                                com.tdvorak.nothingmodes.engine.model.Condition.Or(
                                    listOf(
                                        com.tdvorak.nothingmodes.engine.model.Condition
                                            .Charging(true),
    ...
    @Test
    fun `import JSON with extra unknown fields succeeds (forward compatibility)`() =
        runTest {
            val store = InMemoryAutomationStore()
            val service = ImportExportService(store) { 1000L }
    ...
    @Test
    fun `import JSON with null automations field returns parse error`() =
        runTest {
            val store = InMemoryAutomationStore()
            val service = ImportExportService(store) { 1000L }
