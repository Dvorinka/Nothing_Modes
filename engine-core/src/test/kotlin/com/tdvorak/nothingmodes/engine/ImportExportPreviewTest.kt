...
class ImportExportPreviewTest {
    ...
    private fun automation(id: String = "a1") = Automation(
        id = AutomationId(id),
        name = "Test $id",
    ...
    @Test
    fun `preview parses bundle and derives capabilities without persisting`() = runTest {
    ...
    @Test
    fun `preview rejects unsupported schema version`() = runTest {
    ...
    @Test
    fun `preview rejects malformed json`() = runTest {
        val service = ImportExportService(InMemoryAutomationStore())
    ...
    @Test
    fun `legacy bundle without new fields still imports`() = runTest {
        val store = InMemoryAutomationStore()
