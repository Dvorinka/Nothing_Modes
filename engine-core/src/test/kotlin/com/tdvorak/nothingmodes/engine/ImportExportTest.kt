...
class ImportExportTest {
    ...
    @Test
    fun `export produces valid JSON with all automations`() =
        runTest {
    ...
    @Test
    fun `export by IDs selects only specified automations`() =
        runTest {
            val store = InMemoryAutomationStore()
            store.save(makeAutomation("mode-sleep", "Sleep"))
            store.save(makeAutomation("mode-work", "Work Focus"))
    ...
    @Test
    fun `import restores automations from valid JSON`() =
        runTest {
            val store = InMemoryAutomationStore()
            store.save(makeAutomation("mode-sleep", "Sleep"))
            val service = ImportExportService(store) { 1000L }
    ...
    @Test
    fun `import overwrites existing automations when overwrite is true`() =
        runTest {
            val store = InMemoryAutomationStore()
            store.save(makeAutomation("mode-sleep", "Sleep"))
            val service = ImportExportService(store) { 1000L }
    ...
    @Test
    fun `export-import round trip preserves automation data`() =
        runTest {
            val store = InMemoryAutomationStore()
            val original =
                Automation(
