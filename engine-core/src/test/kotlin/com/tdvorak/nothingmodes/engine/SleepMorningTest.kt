...
class SleepMorningTest {
    @Test
    fun `Sleep mode activates on time trigger with DND, dark mode, extra dim, brightness, glyph off`() =
        runTest {
    ...
    @Test
    fun `Morning routine deactivates Sleep with DND off, extra dim off, brightness restore, glyph restore`() =
        runTest {
            val store = InMemoryAutomationStore()
            val morning =
                Automation(
    ...
    @Test
    fun `Sleep mode with conditions - only fires when battery above 10 and not charging`() =
        runTest {
            val store = InMemoryAutomationStore()
            val sleep =
                Automation(
    ...
    @Test
    fun `TimeWindow mode fires on window start event`() =
        runTest {
            val store = InMemoryAutomationStore()
            val sleep =
                Automation(
    ...
    @Test
    fun `cooldown suppresses repeated firing within cooldown window`() =
        runTest {
            val store = InMemoryAutomationStore()
            val automation =
                Automation(
    ...
    @Test
    fun `action count and execution status are correctly reported`() =
        runTest {
            val store = InMemoryAutomationStore()
            val automation =
                Automation(
