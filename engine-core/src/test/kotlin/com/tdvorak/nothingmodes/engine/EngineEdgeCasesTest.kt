...
class EngineEdgeCasesTest {
    ...
    private fun bootEnvelope(id: String = "evt-1") =
        TriggerEnvelope(
            id = id,
    ...
    @Test
    fun `empty action list produces success outcome with zero results`() =
        runTest {
    ...
    @Test
    fun `all actions fail produces failure outcome`() =
        runTest {
            val store = InMemoryAutomationStore()
            store.save(
                makeAutomation(
    ...
    @Test
    fun `mixed success and failure produces partial failure outcome`() =
        runTest {
            val store = InMemoryAutomationStore()
            store.save(
                makeAutomation(
                    ...
                    executor =
                        ActionExecutor { _, _ ->
                            if (callCount.incrementAndGet() == 2) {
                                ActionResult.Failure("middle_failed")
                            } else {
    ...
    @Test
    fun `mixed success failure reports failed execution status`() =
        runTest {
            val store = InMemoryAutomationStore()
            store.save(
                makeAutomation(
            ...
            val engine =
                Engine(
                    store = store,
                    executor =
                        ActionExecutor { _, ctx ->
                            if (ctx.actionIndex == 1) {
                                ActionResult.Failure("fail")
                            } else {
                                ActionResult.Success
                            }
                        },
    ...
    @Test
    fun `empty action list reports completed execution status`() =
        runTest {
            val store = InMemoryAutomationStore()
    ...
    @Test
    fun `all actions fail reports failed execution status`() =
        runTest {
            val store = InMemoryAutomationStore()
            store.save(
                makeAutomation(
    ...
    @Test
    fun `no armed automations produces empty outcome list`() =
        runTest {
            val store = InMemoryAutomationStore()
            // Store is empty
            val engine = Engine(store = store, executor = ActionExecutor { _, _ -> ActionResult.Success })
    ...
    @Test
    fun `pending approval automation does not fire`() =
        runTest {
            val store = InMemoryAutomationStore()
            store.save(
                makeAutomation("pending", actions = listOf(Action.SetDnd(DndMode.OFF)))
    ...
    @Test
    fun `executor exception is caught and recorded as failure`() =
        runTest {
            val store = InMemoryAutomationStore()
    ...
    @Test
    fun `multiple armed automations fire on broadcast event`() =
        runTest {
            val store = InMemoryAutomationStore()
