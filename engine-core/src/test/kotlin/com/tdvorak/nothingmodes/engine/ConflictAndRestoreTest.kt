...
class ConflictAndRestoreTest {
    ...
    private fun envelope(
        id: String,
        event: TriggerEvent,
    ) = TriggerEnvelope(id = id, event = event, receivedAtMillis = 1000L)

    @Test
    fun `higher priority mode executes first`() =
        runTest {
    ...
    @Test
    fun `mode activation records to mode activation sink`() =
        runTest {
            val store = InMemoryAutomationStore()
    ...
    @Test
    fun `mode deactivation records to mode activation sink`() =
        runTest {
            val store = InMemoryAutomationStore()
            val mode = makeMode("mode-sleep", "Sleep", priority = 10, actions = listOf(Action.SetDnd(DndMode.PRIORITY)))
            store.save(mode)
    ...
    @Test
    fun `mode window start snapshots affected settings`() =
        runTest {
            val store = InMemoryAutomationStore()
            val mode =
                makeMode(
    ...
    @Test
    fun `mode window end restores previous settings via WriteSetting`() =
        runTest {
            val store = InMemoryAutomationStore()
            val mode =
                makeMode(
    ...
    @Test
    fun `condition not met prevents execution`() =
        runTest {
            val store = InMemoryAutomationStore()
            val mode =
                makeMode(
    ...
    @Test
    fun `cooldown suppresses duplicate firing`() =
        runTest {
            val store = InMemoryAutomationStore()
            val automation =
                Automation(
    ...
    @Test
    fun `failed action records failed execution status`() =
        runTest {
            val store = InMemoryAutomationStore()
    ...
    @Test
    fun `routine does not snapshot or restore`() =
        runTest {
            val store = InMemoryAutomationStore()
            val routine =
                Automation(
    ...
    @Test
    fun `mode window end without snapshots does not crash`() =
        runTest {
            val store = InMemoryAutomationStore()
    ...
    @Test
    fun `current mode active condition evaluates with state`() =
        runTest {
            val store = InMemoryAutomationStore()
            val mode =
                makeMode(
