...
import com.dvoranka.nothingmodes.engine.model.CreatedBy
import com.dvoranka.nothingmodes.engine.model.DndMode
import com.dvoranka.nothingmodes.engine.model.EngineJson
import com.dvoranka.nothingmodes.engine.model.NightMode
...
import com.dvoranka.nothingmodes.engine.runtime.ConditionEvaluator
import com.dvoranka.nothingmodes.engine.runtime.DeviceState
import com.dvoranka.nothingmodes.engine.runtime.Engine
import com.dvoranka.nothingmodes.engine.runtime.FirePolicy
import com.dvoranka.nothingmodes.engine.runtime.InMemoryAutomationStore
import com.dvoranka.nothingmodes.engine.runtime.NoopActionExecutor
import com.dvoranka.nothingmodes.engine.runtime.NoopAuditSink
import com.dvoranka.nothingmodes.engine.runtime.NoopExecutionJournal
import com.dvoranka.nothingmodes.engine.runtime.StableExecutionIdFactory
...
import com.dvoranka.nothingmodes.engine.runtime.TriggerMatcher
import com.dvoranka.nothingmodes.engine.runtime.CronSchedule
...
class EngineTest {
    ...
    fun `action serialization round trip`() {
        val actions: List<Action> = listOf(
            ...
            Action.SetVolume(com.dvoranka.nothingmodes.engine.model.VolumeStream.MEDIA, 50),
            Action.SetWifi(true),
    ...
    fun `trigger serialization round trip`() {
        val triggers: List<Trigger> = listOf(
            ...
            Trigger.PhoneState(
                event = com.dvoranka.nothingmodes.engine.model.PhoneEvent.SMS_RECEIVED,
                textMatch = "OTP",
            ),
            Trigger.Connectivity(
                medium = com.dvoranka.nothingmodes.engine.model.ConnMedium.WIFI,
                state = com.dvoranka.nothingmodes.engine.model.ConnState.CONNECTED,
            ),
            Trigger.Boot,
            Trigger.BatteryLevel(level = 20),
            Trigger.ScreenStateTrigger(com.dvoranka.nothingmodes.engine.model.ScreenState.OFF),
    ...
    fun `condition serialization round trip`() {
        val conditions: List<Condition> = listOf(
            ...
            Condition.BatteryLevel(com.dvoranka.nothingmodes.engine.model.CmpOp.LT, 20),
            Condition.Charging(true),
            Condition.WifiConnected("MyHomeWifi"),
            Condition.BluetoothConnected(),
            Condition.ScreenStateCondition(com.dvoranka.nothingmodes.engine.model.ScreenState.ON),
            Condition.CurrentModeActive("mode-work"),
            Condition.AppInForeground("com.android.chrome"),
            Condition.And(listOf(
                Condition.BatteryLevel(com.dvoranka.nothingmodes.engine.model.CmpOp.GT, 50),
    ...
    fun `automation serialization round trip`() {
        val automation = Automation(
            ...
            conditions = Condition.And(listOf(
                Condition.BatteryLevel(com.dvoranka.nothingmodes.engine.model.CmpOp.GT, 10),
                Condition.Not(Condition.Charging(true)),
            )),
        )
    ...
    fun `condition evaluator - battery level`() {
        ...
        val low = Condition.BatteryLevel(com.dvoranka.nothingmodes.engine.model.CmpOp.LT, 20)
        assertEquals(ConditionEvaluator.Result.MET, evaluator.result(low, state))

        val high = Condition.BatteryLevel(com.dvoranka.nothingmodes.engine.model.CmpOp.GT, 50)
