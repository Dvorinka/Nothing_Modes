...
- [~] **Condition parity pass** — Added generic `Condition.BooleanState`, `Condition.NumericState`, and `Condition.AtLocation` with a closed `StateKeys` registry. Covered: device locked, Wi-Fi/Bluetooth/mobile data/AOD/DND radios, brightness, refresh rate, screen timeout, at-location radius. Still to add: `TorchOn`, `HotspotOn`, `NotificationPresent`, `EventActive`.
