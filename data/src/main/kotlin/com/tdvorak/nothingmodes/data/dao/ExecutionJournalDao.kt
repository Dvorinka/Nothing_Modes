...
interface ExecutionJournalDao {
    ...
    @Query(
        "SELECT * FROM action_results WHERE executionId IN (SELECT executionId FROM fire_claims WHERE automationId = :automationId) ORDER BY atMillis DESC LIMIT :limit",
    )
    suspend fun forAutomation(
        automationId: String,
        limit: Int = 500,
    ): List<ActionResultEntity>
