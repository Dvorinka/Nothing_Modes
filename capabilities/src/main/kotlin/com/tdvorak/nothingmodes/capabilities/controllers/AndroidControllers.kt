...
class AndroidBrightnessController(
    ...
    override suspend fun isAutoBrightness(): Boolean? =
        try {
...
class AndroidExtraDimController(
    private val context: Context,
) : ExtraDimController {
    ...
    override suspend fun isExtraDimEnabled(): Boolean? =
        try {
...
class AndroidVolumeController(
    ...
    override suspend fun getVolume(stream: VolumeStream): Int? =
        try {
            val androidStream =
                when (stream) {
                    VolumeStream.MEDIA -> AudioManager.STREAM_MUSIC
...
class AndroidScreenTimeoutController(
    private val context: Context,
    ...
    override suspend fun getScreenTimeout(): Int? =
        try {
...
class AndroidDarkModeController(
    private val context: Context,
) : DarkModeController {
    private val uiModeManager = context.getSystemService(UiModeManager::class.java)

    override suspend fun setDarkMode(mode: NightMode): ControllerResult =
        try {
            when (mode) {
    ...
    override suspend fun getDarkMode(): NightMode? =
        try {
...
class AndroidScreenStateController(
    private val context: Context,
