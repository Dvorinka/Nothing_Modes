...
class CustomGlyphStore(
    ...
    fun names(): List<String> = read().keys().asSequence().toList().sorted()
    ...
    fun design(name: String): GlyphFrameCodec.Design? =
        designJson(name)?.let { runCatching { GlyphFrameCodec.decode(it) }.getOrNull() }
     ...
     */
    fun save(name: String, designJson: String): Boolean {
     ...
     */
    fun saveFrame(name: String, frame: IntArray, size: Int = 25): Boolean =
        save(name, GlyphFrameCodec.encode(listOf(frame), size))
     ...
     */
    fun import(designJson: String, suggestedName: String?): String? {
