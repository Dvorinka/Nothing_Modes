...
import com.tdvorak.nothingmodes.engine.model.StateQuery
...
class ParametricStateReaderTest {
    @Test
    fun `all query families execute bounded argv commands and return opaque ids`() =
        runTest {
    ...
    @Test
    fun `invalid parameters and sysfs escaping never reach the shell`() =
        runTest {
    ...
    @Test
    fun `missing settings and ambiguous dumpsys fields remain unavailable`() =
        runTest {
    ...
    @Test
    fun `truncated timed out failed and non scalar outputs remain unavailable`() =
        runTest {
    ...
    @Test
    fun `multiple fields from one dumpsys service share one bounded process`() =
        runTest {
    ...
    @Test
    fun `cancellation propagates without being converted to unknown`() =
        runTest {
    ...
    private companion object {
        fun command(
            binary: String,
            vararg args: String,
        ): List<String> = listOf("/system/bin/$binary", *args)
