...
class StateReader(
    ...
    internal companion object {
        ...
        private val RESUMED_ACTIVITY_FIELD =
            Regex(
        ...
        fun parseRinger(raw: String): String? =
            when (raw.trim()) {
                "0" -> "silent"
                "1" -> "vibrate"
                "2" -> "normal"
        ...
        fun parseWifi(raw: String): String? =
            when (raw.trim()) {
                "0", "3" -> "off"
                "1", "2" -> "on"
                else -> null
        ...
        fun parseBluetooth(raw: String): String? =
            when (raw.trim()) {
                "0" -> "off"
                "1", "2" -> "on"
                else -> null
        ...
        fun parseDnd(raw: String): String? =
            when (raw.trim()) {
                "0" -> "off"
                "1" -> "priority"
                "2" -> "total"
        ...
        fun parseBooleanSetting(raw: String): String? =
            when (raw.trim()) {
                "0" -> "off"
                "1" -> "on"
                else -> null
        ...
        fun parseBattery(raw: String): BatterySnapshot {
            ...
            val charging =
                when (fields["status"]?.toIntOrNull()) {
                    2, 5 -> true
                    3, 4 -> false
                    else -> null
                }
        ...
        fun parseForegroundPackage(raw: String): String? =
            raw
                .lineSequence()
