package com.tdvorak.nothingmodes.device
...
import com.tdvorak.nothingmodes.engine.model.WriteSettingPolicy
import com.tdvorak.nothingmodes.engine.runtime.ExecutionId
import com.tdvorak.nothingmodes.shizuku.PrivilegedShell
import com.tdvorak.nothingmodes.shizuku.ShellResult
...
enum class RingerMode(
    internal val shellValue: String,
) {
    NORMAL("NORMAL"),
    VIBRATE("VIBRATE"),
    SILENT("SILENT"),
    ...
    companion object {
        fun fromEngineValue(value: String): RingerMode? =
            entries.firstOrNull {
...
interface DeviceController {
     ...
     * percorso BASE ([com.tdvorak.nothingmodes.automation.base.BaseActionSurface]) queste girano con identità
...
class DeviceTools(
    ...
    override suspend fun setBluetooth(
        on: Boolean,
        executionId: ExecutionId,
        priority: Int,
    ) {
    ...
    override suspend fun setMobileData(
        on: Boolean,
        executionId: ExecutionId,
        priority: Int,
    ) {
    ...
    override suspend fun setRinger(
        mode: RingerMode,
        executionId: ExecutionId,
        priority: Int,
    ) {
    ...
    private fun settingsAction(screen: SettingsScreen): String =
        when (screen) {
    ...
    private fun validHttpUrl(raw: String): Boolean =
        runCatching {
...
internal fun extractUiXmlPayload(raw: String): String? {
    ...
    val rootEnd =
        if (displaysStart != null && displaysStart >= start) {
            "</displays>"
        } else {
            "</hierarchy>"
        }
